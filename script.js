...
// ✅ تحويل واجهة العرض إلى صفحة كاملة باحترافية وتصميم عصري فاخر (Premium UI)
function openDoctorProfileModal(doc, doctorName) {
  ...
  // 1. بناء جدول العمل الأسبوعي (تصميم أنظف وأرقى)
  let scheduleHtml = '';
  if (doc.working_days && Object.keys(doc.working_days).length > 0) {
    try {
      ...
      for(let i=0; i<=6; i++) {
        if(wd[i] && wd[i].active) {
          scheduleHtml += `<div style="display:flex; justify-content:space-between; align-items: center; padding: 0.6rem 0; font-size: 0.95rem; border-bottom: 1px solid #f1f5f9;">
            <span style="color: #64748b; font-weight: 600;">${daysNames[i]}</span>
            <span dir="ltr" style="color: var(--primary-dark); font-weight: 800; background: #f8fafc; padding: 0.3rem 0.75rem; border-radius: 6px; letter-spacing: 0.5px;">${wd[i].start} - ${wd[i].end}</span>
          </div>`;
        }
      }
  ...
  if(!scheduleHtml) {
    scheduleHtml = `<div class="text-sm text-gray" style="padding: 1.5rem 0; text-align: center; background: #f8fafc; border-radius: 8px;">${currentLang === 'ar' ? 'غير متوفر حالياً' : 'Not available currently'}</div>`;
  }

  // 2. بناء قسم الخدمات (تصميم بطاقات عصرية نظيفة)
  let servicesHtml = '';
  if (doc.services && Array.isArray(doc.services) && doc.services.length > 0) {
      servicesHtml = `
      <div style="margin-top: 2rem; background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 4px 15px rgba(0,0,0,0.03); border: 1px solid rgba(0,0,0,0.05);">
        <h3 style="margin: 0 0 1.5rem 0; font-size: 1.2rem; font-weight: 800; color: #0f172a; display: flex; align-items: center; gap: 0.5rem;">
           <span style="background: rgba(14, 165, 233, 0.1); color: var(--primary); padding: 0.5rem; border-radius: 8px; display: flex;">
        ...
        <div style="display: grid; gap: 1rem;">
            ${doc.services.map(service => `
                <div style="background: #f8fafc; border-radius: 10px; padding: 1.25rem;">
                    <div style="color: var(--primary-dark); font-size: 1.05rem; font-weight: 800; margin-bottom: ${service.items.length > 0 ? '0.75rem' : '0'}; display: flex; align-items: center; gap: 8px;">
                        <span style="display: inline-block; width: 8px; height: 8px; background: var(--primary); border-radius: 50%;"></span>
                        ${escapeHtml(service.category)}
                    </div>
                    ${service.items.length > 0 ? `
                        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 0.5rem; padding-inline-start: 1.25rem;">
                            ${service.items.map(item => `
                                <div style="color: #475569; font-size: 0.9rem; font-weight: 500; display: flex; align-items: center; gap: 6px;">
  ...
  document.getElementById('fullProfileContent').innerHTML = `
    <div class="grid grid-1 grid-2" style="gap: 2rem; align-items: start;">
      ...
      <div style="display: flex; flex-direction: column; gap: 1.5rem;">
        ...
        <div style="background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 4px 15px rgba(0,0,0,0.03); border: 1px solid rgba(0,0,0,0.05);">
            
            <div style="display: flex; align-items: flex-start; gap: 1rem; margin-bottom: 1.5rem;">
              <div style="background: rgba(14, 165, 233, 0.1); color: var(--primary); padding: 0.75rem; border-radius: 12px; flex-shrink: 0;">
              ...
              <div>
                <div style="font-size: 0.75rem; color: #64748b; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0.2rem;">${currentLang === 'ar' ? 'العنوان الدقيق' : 'Exact Location'}</div>
                <div style="font-weight: 800; color: #0f172a; font-size: 1.05rem; line-height: 1.4;">${escapeHtml(doc.exact_location)}, <span style="color: var(--primary);">${escapeHtml(t(doc.municipality))}</span></div>
              </div>
            </div>
            
            <div style="width: 100%; height: 1px; background: #f1f5f9; margin-bottom: 1.5rem;"></div>
            
            <div style="display: flex; align-items: flex-start; gap: 1rem;">
              <div style="background: rgba(16, 185, 129, 0.1); color: #10b981; padding: 0.75rem; border-radius: 12px; flex-shrink: 0;">
                <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>
              </div>
              <div>
                <div style="font-size: 0.75rem; color: #64748b; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0.2rem;">${currentLang === 'ar' ? 'رقم العيادة' : 'Clinic Phone'}</div>
                <div style="font-weight: 800; color: #0f172a; font-size: 1.15rem; letter-spacing: 1px;" dir="ltr">${escapeHtml(formatPhoneNumber(doc.phone))}</div>
              </div>
            </div>
        </div>

        <div style="background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 4px 15px rgba(0,0,0,0.03); border: 1px solid rgba(0,0,0,0.05);">
          <div style="font-size: 1.1rem; font-weight: 800; color: #0f172a; margin-bottom: 1.25rem; display: flex; gap: 0.5rem; align-items: center;">
            <span style="background: rgba(14, 165, 233, 0.1); color: var(--primary); padding: 0.5rem; border-radius: 8px; display: flex;">
        ...
        <div style="display: flex; gap: 0.75rem; flex-wrap: wrap;">
          ${doc.whatsapp_number ? `<a href="https://wa.me/${doc.whatsapp_number.replace(/\D/g, '')}" target="_blank" class="btn" style="background: white; color: #059669; border: 1px solid #a7f3d0; padding: 0.85rem; font-size: 0.95rem; flex: 1; justify-content: center; font-weight: 800; box-shadow: 0 2px 8px rgba(16,185,129,0.05); transition: all 0.2s;" onmouseover="this.style.background='#10b981'; this.style.color='white'; this.style.transform='translateY(-2px)';" onmouseout="this.style.background='white'; this.style.color='#059669'; this.style.transform='translateY(0)';"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"></path></svg> ${t('whatsappBtn')}</a>` : ''}
          ${fbLink ? `<a href="${fbLink}" target="_blank" class="btn" style="background: white; color: #2563eb; border: 1px solid #bfdbfe; padding: 0.85rem; font-size: 0.95rem; flex: 1; justify-content: center; font-weight: 800; box-shadow: 0 2px 8px rgba(59,130,246,0.05); transition: all 0.2s;" onmouseover="this.style.background='#3b82f6'; this.style.color='white'; this.style.transform='translateY(-2px)';" onmouseout="this.style.background='white'; this.style.color='#2563eb'; this.style.transform='translateY(0)';"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"></path></svg> ${t('facebookBtn')}</a>` : ''}
          ${doc.contact_email ? `<a href="mailto:${doc.contact_email}" class="btn" style="background: white; color: #475569; border: 1px solid #cbd5e1; padding: 0.85rem; font-size: 0.95rem; flex: 1; justify-content: center; font-weight: 800; box-shadow: 0 2px 8px rgba(0,0,0,0.05); transition: all 0.2s;" onmouseover="this.style.background='#475569'; this.style.color='white'; this.style.transform='translateY(-2px)';" onmouseout="this.style.background='white'; this.style.color='#475569'; this.style.transform='translateY(0)';"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg> ${t('emailBtn')}</a>` : ''}
        </div>
      </div>
      ...
      <div style="display: flex; flex-direction: column; gap: 1.5rem;">
        ...
        ${doc.map_link ? `
        <div style="background: white; border: 1px solid #fecaca; border-radius: 16px; padding: 1.5rem; text-align: center; box-shadow: 0 4px 15px rgba(239, 68, 68, 0.05);">
          <div style="font-size: 0.85rem; font-weight: 800; color: #ef4444; margin-bottom: 1rem; text-transform: uppercase; letter-spacing: 0.5px;">${t('mapLocation')}</div>
          <a href="${doc.map_link}" target="_blank" class="btn" style="width: 100%; justify-content: center; background: #fff5f5; color: #ef4444; border: 2px dashed #fca5a5; padding: 1rem; font-weight: 800; font-size: 1.05rem; border-radius: 12px; transition: all 0.2s;" onmouseover="this.style.background='#ef4444'; this.style.color='white'; this.style.borderStyle='solid';" onmouseout="this.style.background='#fff5f5'; this.style.color='#ef4444'; this.style.borderStyle='dashed';">
        ...
        <div style="background: white; padding: 2rem; border-radius: 16px; border: 1px solid rgba(0,0,0,0.05); text-align: center; box-shadow: 0 4px 15px rgba(0,0,0,0.03);">
            <div style="color: #0f172a; font-weight: 800; font-size: 1.1rem; margin-bottom: 1.25rem;">
                <span style="background: rgba(14, 165, 233, 0.1); color: var(--primary); padding: 0.4rem; border-radius: 8px; display: inline-flex; vertical-align: bottom; margin-inline-end: 6px;">
            ...
            <div style="display: inline-block; padding: 1rem; background: #f8fafc; border: 1px dashed #cbd5e1; border-radius: 12px;">
                <img 
                  src="${qrPrimary}" 
                  data-fallback-url="${qrFallback}" 
                  onerror="handleQrError(this)" 
                  alt="QR Contact Code" 
                  style="width: 160px; height: 160px; display: block; margin: 0 auto; mix-blend-mode: multiply;" 
                />
            </div>
        </div>
      </div>
    ...
    ${doc.extra_info ? `
    <div style="margin-top: 2rem; background: #fefce8; border: 1px solid #fde68a; border-radius: 16px; padding: 1.5rem; box-shadow: 0 4px 15px rgba(245, 158, 11, 0.05);">
      <h4 style="font-size: 1.1rem; font-weight: 800; margin-bottom: 0.75rem; color: #d97706; display: flex; align-items: center; gap: 0.5rem;">
        <span style="background: rgba(245, 158, 11, 0.15); padding: 0.4rem; border-radius: 8px; display: flex;">
      ...
      <p style="color: #92400e; font-size: 1rem; line-height: 1.6; margin: 0; font-weight: 500;">${escapeHtml(doc.extra_info)}</p>
    </div>` : ''}

    <div style="position: sticky; bottom: 0; background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); margin: 3rem -2rem -2rem -2rem; padding: 1.25rem 2rem; border-top: 1px solid rgba(0,0,0,0.08); box-shadow: 0 -10px 30px rgba(0,0,0,0.05); display: flex; gap: 1rem; flex-wrap: wrap; z-index: 50;">
      
      <div style="display: flex; gap: 1rem; flex: 1; min-width: 280px;">
          <button class="btn" style="background: white; border: 1px solid var(--border); color: #0f172a; padding: 0.85rem; font-size: 0.95rem; flex: 1; font-weight: 800; box-shadow: 0 2px 5px rgba(0,0,0,0.02); transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'" onclick="openReviewsModal('${doc.id}', '${escapeHtml(doc.first_name)} ${escapeHtml(doc.last_name)}')">
            <span style="color: #f59e0b; margin-inline-end: 6px; font-size: 1.2rem;">★</span> ${currentLang === 'ar' ? 'التقييمات' : 'Reviews'}
          </button>
          
          <button class="btn" style="background: white; border: 1px solid var(--border); color: #0f172a; padding: 0.85rem; font-size: 0.95rem; flex: 1; font-weight: 800; box-shadow: 0 2px 5px rgba(0,0,0,0.02); transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'" onclick="navigator.clipboard.writeText('${profileUrl}'); showToast(currentLang==='ar'?'تم نسخ الرابط بنجاح':'Copied', 'success');">
            <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="var(--primary)" stroke-width="2.5" style="margin-inline-end: 6px; vertical-align: sub;"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>
            ${shareText}
          </button>
      </div>

      <button class="btn ${isBookingEnabled ? 'btn-success' : 'btn-secondary'}" style="flex: 1.5; padding: 0.85rem; font-size: 1.1rem; font-weight: 800; letter-spacing: 0.5px; box-shadow: ${isBookingEnabled ? '0 8px 20px rgba(16, 185, 129, 0.25)' : 'none'}; min-width: 250px; transition: transform 0.2s;" ${isBookingEnabled ? `onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'" onclick="openBooking('${doc.id}')"` : 'disabled'}>
