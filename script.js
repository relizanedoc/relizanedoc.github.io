...
// ✅ الدالة الجديدة لعرض تفاصيل الطبيب (مع QR Code لحفظ جهة الاتصال - بدون أي فرنسية)
function openDoctorProfileModal(doc, doctorName) {
  ...
  content.innerHTML = `
    ...
    <div class="dp-body">
      ...
      <div class="dp-info-row">
        ...
        <div style="flex: 1;">
          ...
          <div style="background: var(--surface); padding: 0.75rem; border-radius: 8px; border: 1px solid var(--border);">${scheduleHtml}</div>
        </div>
      </div>
      ...
      <div class="dp-info-row" style="border-top: 1px dashed var(--border); padding-top: 1rem;">
        ...
        <div>
          ...
          <div style="color: var(--text); font-size: 0.9rem; line-height: 1.5;">${escapeHtml(doc.extra_info)}</div>
        </div>
      </div>` : ''}
      ...
      <div style="display: flex; gap: 0.75rem; margin-top: 1.5rem;">
        <button class="btn btn-secondary" style="flex: 1; padding: 0.5rem; font-size: 0.85rem;" onclick="openReviewsModal('${doc.id}', '${escapeHtml(doc.first_name)} ${escapeHtml(doc.last_name)}')">
    ...
    <div class="dp-footer">
      <button class="btn ${isBookingEnabled ? 'btn-success' : 'btn-secondary'} btn-block" ${isBookingEnabled ? `onclick="document.getElementById('doctorProfileModal').classList.add('hidden'); openBooking('${doc.id}')"` : 'disabled'}>
