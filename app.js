// ========================================================================
// 1. CONFIGURATION & ADAPTERS (Supabase & Firebase)
// ========================================================================
  ...
  try {
    switch (action) {
      ...
      case 'addDoctor':
        ...
        const { error: errAdd } = await supabase.from('directory').insert([{
            doctor_id: newDocId, first_name: payload.FirstName, last_name: payload.LastName,
            specialty: payload.Specialty, municipality: payload.Municipality, phone: payload.Phone,
            exact_location: payload.ExactLocation, extra_info: payload.ExtraInfo || '',
            password: '0000', booking_enabled: true
          }]);
        if (errAdd) throw errAdd;
        return { success: true, data: { DoctorID: newDocId } };

      case 'bookAppointment':
        const { data: booked, error: errBook } = await supabase.from('bookings').insert([{
            doctor_id: payload.DoctorID, patient_name: payload.PatientName, patient_phone: payload.PatientPhone,
            user_email: payload.userEmail || null, appointment_date: payload.AppointmentDate,
            appointment_time: payload.AppointmentTime, status: 'قيد الانتظار'
          }]).select('booking_id').single();
        if (errBook) throw errBook;
        return { success: true, data: { BookingID: booked.booking_id } };

      case 'doctorLogin':
        const { data: loginData, error: errLogin } = await supabase.from('directory')
          .select('*').eq('doctor_id', payload.doctorId).eq('phone', payload.phone).single();
        ...
        return {
          success: true,
          data: {
            sessionToken: 'token_' + Date.now(), doctorName: loginData.first_name + ' ' + loginData.last_name,
            workingDays: loginData.working_days, bookingEnabled: loginData.booking_enabled,
            appointments: (appts || []).map(a => ({
              BookingID: a.booking_id, PatientName: a.patient_name, PatientPhone: a.patient_phone,
              AppointmentDate: a.appointment_date, AppointmentTime: a.appointment_time, Status: a.status, UserEmail: a.user_email
            }))
          }
        };
      ...
      case 'getBookingStatus':
        const { data: trackData, error: errTrack } = await supabase.from('bookings')
          .select('booking_id, patient_name, appointment_date, appointment_time, status')
          .eq('booking_id', payload.bookingId).eq('patient_phone', payload.phone).single();
      ...
      case 'getReviews':
        const { data: revData, error: errRev } = await supabase.from('reviews')
          .select('*').eq('doctor_id', payload.doctorId).order('created_at', { ascending: false });
        if (errRev) throw errRev;
        const mappedReviews = (revData || []).map(r => ({
           reviewId: r.review_id, patientName: r.patient_name, rating: r.rating, comment: r.comment,
           date: new Date(r.created_at).toLocaleDateString(), isOwner: (payload.userEmail && r.user_email === payload.userEmail)
        }));
        return { success: true, data: mappedReviews };

      case 'addReview':
        const { error: errAddRev } = await supabase.from('reviews').insert([{
            doctor_id: payload.doctorId, patient_name: payload.patientName, user_email: payload.userEmail || 'anonymous',
      ...
      case 'getUserBookings':
        const { data: uBookings, error: errUBook } = await supabase.from('bookings')
          .select('booking_id, doctor_id, patient_name, appointment_date, appointment_time, status, directory(first_name, last_name)')
          .eq('user_email', payload.userEmail || payload.idToken); 
        if (errUBook) throw errUBook;
        const mappedUserBookings = (uBookings || []).map(b => ({
           bookingId: b.booking_id, patientName: b.patient_name, date: b.appointment_date, time: b.appointment_time, status: b.status,
           doctorName: b.directory ? (b.directory.first_name + ' ' + b.directory.last_name) : 'طبيب'
        }));
        return { success: true, data: mappedUserBookings };

      case 'sendEmail':
        console.log('ميزة الإيميل معطلة مؤقتاً ريثما يتم دمج EmailJS. البيانات:', payload);
        return { success: true };
...
const i18n = {
  en: {
    ...
    chatFoundPrefix: 'I found ', chatFoundSuffix: ' doctors based on your request. Here are the top 3 results:',
    chatExactResults: 'Here are the results matching your question:', chatDoctorLabel: 'Doctor: ',
    chatSpecLabel: 'Specialty: ', chatPhoneLabel: 'Phone: ', chatMunLabel: 'Municipality: ',
    chatAddressLabel: 'Address: ', chatBookDetailsBtn: 'View Details & Book',
  ...
  ar: {
    ...
    chatExactResults: 'تفضل، لقد وجدت هذه النتائج المطابقة لسؤالك:', chatDoctorLabel: 'الدكتور(ة): ',
    chatSpecLabel: 'الاختصاص: ', chatPhoneLabel: 'الهاتف: ', chatMunLabel: 'البلدية: ',
    chatAddressLabel: 'العنوان: ', chatBookDetailsBtn: 'عرض التفاصيل والحجز',
...
function openDoctorProfileModal(doc, doctorName) {
  ...
  if (doc.WorkingDays) {
     try {
        const wd = typeof doc.WorkingDays === 'string' ? JSON.parse(doc.WorkingDays) : doc.WorkingDays;
...
function openBooking(doctorId) {
  ...
  if (currentDoctor.WorkingDays) {
     try {
        wd = typeof currentDoctor.WorkingDays === 'string' ? JSON.parse(currentDoctor.WorkingDays) : currentDoctor.WorkingDays;
...
function confirmBooking() {
  ...
  const dayNum = selectedDate.getDay();
  let wd = {};
  if (currentDoctor.WorkingDays) { try { wd = typeof currentDoctor.WorkingDays === 'string' ? JSON.parse(currentDoctor.WorkingDays) : currentDoctor.WorkingDays; } catch(err) {} }
...
function renderDashboardUI(data, doctorId) {
  ...
  let savedDays = {};
  try { savedDays = typeof data.workingDays === 'string' ? JSON.parse(data.workingDays) : (data.workingDays || {}); } catch(e) {}
  ...
  const isEnabled = String(data.bookingEnabled).toUpperCase() === 'TRUE' || data.bookingEnabled === true;
...
async function handleDashboardLogin(e) {
  ...
  try {
    ...
    if (!result.success) throw new Error(result.error);
    resetLoginAttempts();
    localStorage.setItem('doctorSession', JSON.stringify({ doctorId, phone, sessionToken: result.data.sessionToken }));
...
window.changeBookingStatus = async function(bookingId, newStatus, patientEmail, doctorName, apptDate) {
  ...
  try {
    const result = await apiPost('updateBookingStatus', { doctorId: session.doctorId, sessionToken: session.sessionToken, bookingId: bookingId, newStatus: newStatus });
    if (result.success) {
...
window.saveWorkingHours = async function() {
  ...
  try {
    const result = await apiPost('updateDoctorHours', { doctorId: session.doctorId, sessionToken: session.sessionToken, workingDays: JSON.stringify(workingDaysData) });
    if(result.success) {
       showToast('تم حفظ أوقات وأيام العمل بنجاح', 'success');
       const docIndex = allDoctors.findIndex(d => d.DoctorID === session.doctorId);
       if(docIndex > -1) allDoctors[docIndex].WorkingDays = JSON.stringify(workingDaysData);
    } else throw new Error(result.error);
...
async function handleToggleBooking(e) {
  ...
  try {
    ...
    if (docIndex > -1) allDoctors[docIndex].BookingEnabled = isChecked ? 'TRUE' : 'FALSE';
  } catch (err) {
  ...
  try {
    ...
    bookings.forEach(b => {
        ...
        if (b.status === 'مؤكد') {
            ...
            statusIndicator = '#10b981'; displayStatus = t('statusConfirmed');
        } else if (b.status === 'ملغى') {
            statusStyle = 'background: #fef2f2; color: #ef4444; border: 1px solid #fecaca;';
            statusIndicator = '#ef4444'; displayStatus = t('statusCancelled');
        }
        html += `
        <div class="card-hover" style="background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 1.25rem; position: relative; overflow: hidden; box-shadow: var(--shadow-sm);">
            <div style="position: absolute; right: 0; top: 0; bottom: 0; width: 4px; background: ${statusIndicator};"></div>
...
function setLang(lang) {
  ...
  setTimeout(() => {
      if (window.tsAddSpecialty && addSpecSel) {
          const val = window.tsAddSpecialty.getValue(); window.tsAddSpecialty.clearOptions();
      ...
      if (window.tsAddMunicipality && addMunSel) {
          const val = window.tsAddMunicipality.getValue(); window.tsAddMunicipality.clearOptions();
...
window.deleteReview = async function(reviewId, doctorId) {
  ...
  try {
      ...
      const res = await apiPost('deleteReview', { reviewId: reviewId, idToken: idToken });
      if (!res.success) throw new Error(res.error);
...
document.addEventListener('submit', async function(e) {
  if (e.target && e.target.id === 'reviewForm') {
      ...
      try {
          const user = await getCurrentUser();
          const res = await apiPost('addReview', { doctorId: doctorId, patientName: user.Name || user.Email.split('@')[0], rating: parseInt(rating), comment: comment });
          if (!res.success) throw new Error(res.error);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const toggleChat = () => {
      ...
      if (!medicalChatbot.classList.contains('hidden')) chatInputText.focus();
  };
  ...
  const processUserMessage = (rawMsg) => {
      ...
      matchedDoctors = allDoctors.filter(doc => {
          ...
          const docNameEn = normalizeText(doc.FirstName + " " + doc.LastName); 
          const isNameMatch = cleanMsg.split(' ').some(word => word.length > 2 && (docNameAr.includes(word) || docNameEn.includes(word)));
  ...
  if(chatInputText) chatInputText.addEventListener('keypress', (e) => { if (e.key === 'Enter') handleSend(); });
});
...
window.sendBookingEmail = function(recipientEmail, doctorName, appointmentDate, status) {
  // ملاحظة: سيتم استبدال هذه الدالة لاحقاً لتعمل مع EmailJS بدلاً من Apps Script
  console.log("إشعار محاكاة للإيميل:", recipientEmail, doctorName, appointmentDate, status);
  apiPost('sendEmail', { recipient: recipientEmail, subject: status, htmlBody: "..." });
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  bindClick('logoHomeBtn', () => router('home'));
  bindClick('btn-en', () => setLang('en'));
  bindClick('btn-ar', () => setLang('ar'));
  bindClick('googleSignInBtn', handleGoogleSignIn);
  bindClick('authSubmitBtn', handleEmailAuth);
  bindClick('authToggleText', toggleAuthMode);
  bindClick('backToHomeBtn', () => router('home'));
  bindClick('backToDirBtn', () => router('home'));
  bindClick('bookingBtn', confirmBooking);
  bindClick('confirmDialogOkBtn', submitBooking);
  bindClick('cancelDialogBtn', closeConfirmDialog);
  bindClick('dashboardLogoutBtn', logoutDashboard);
  bindClick('logoutBtn', logoutUser);
  bindClick('backToTop', () => window.scrollTo({ top: 0, behavior: 'smooth' }));

  const addDocForm = document.getElementById('addDoctorForm');
  if(addDocForm) addDocForm.onsubmit = handleAddDoctor;
  const dashLoginForm = document.getElementById('dashboardLoginForm');
  if(dashLoginForm) dashLoginForm.onsubmit = handleDashboardLogin;

  const toggleSwitch = document.getElementById('bookingToggleSwitch');
  if(toggleSwitch) toggleSwitch.onchange = handleToggleBooking;

  const searchInput = document.getElementById('searchInput');
  if(searchInput) searchInput.oninput = filterDoctors;
  const specFilter = document.getElementById('specialtyFilter');
  if(specFilter) specFilter.onchange = filterDoctors;
  const munFilter = document.getElementById('municipalityFilter');
  if(munFilter) munFilter.onchange = filterDoctors;

  document.querySelectorAll('.nav-btn[data-nav]').forEach(btn => { btn.onclick = () => router(btn.getAttribute('data-nav')); });

  window.onscroll = () => { const btn = document.getElementById('backToTop'); if(btn) { if (document.body.scrollTop > 300 || document.documentElement.scrollTop > 300) btn.classList.remove('hidden'); else btn.classList.add('hidden'); } };
  ...
  if (typeof TomSelect !== 'undefined') {
    const tsSettings = {
      ...
      render: { no_results: function(data, escape) { return '<div class="no-results">' + (currentLang === 'ar' ? 'لا توجد نتائج' : 'No results found') + '</div>'; } }
    };
    if(document.querySelector("#specialtyFilter")) window.tsSpecialtyFilter = new TomSelect("#specialtyFilter", tsSettings);
    if(document.querySelector("#municipalityFilter")) window.tsMunicipalityFilter = new TomSelect("#municipalityFilter", tsSettings);
    if(document.querySelector('select[name="Specialty"]')) window.tsAddSpecialty = new TomSelect('select[name="Specialty"]', tsSettings);
    if(document.querySelector('select[name="Municipality"]')) window.tsAddMunicipality = new TomSelect('select[name="Municipality"]', tsSettings);
  }
  ...
  if (trackForm) {
    trackForm.addEventListener('submit', async (e) => {
      ...
      setLoading(btn, true); resultDiv.classList.add('hidden');
      try {
        ...
        if (result.success) {
          ...
          resultDiv.innerHTML = `
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-top:1rem; border-top:1px solid var(--border); padding-top:1rem;">
              <span>${currentStatusTxt}</span> <span class="badge" style="${statusStyle}">${escapeHtml(displayStatus)}</span>
            </div>
          `;
          resultDiv.classList.remove('hidden');
        } else { showToast(currentLang === 'ar' ? result.error : 'Booking not found', 'error'); }
  ...
  document.querySelectorAll('.nav-btn').forEach(btn => {
    btn.addEventListener('click', () => { if (window.innerWidth <= 768 && navLinks) navLinks.classList.remove('show'); });
  });
  ...
  if (savedSession) {
    try {
      ...
      apiPost('doctorLogin', { doctorId: session.doctorId, phone: session.phone, sessionToken: session.sessionToken }).then(result => {
          if (result && result.success) {
              localStorage.setItem('doctorSession', JSON.stringify({ doctorId: session.doctorId, phone: session.phone, sessionToken: result.data.sessionToken }));
              document.getElementById('loginSection').classList.add('hidden');
              document.getElementById('dashboardSection').classList.remove('hidden');
              renderDashboardUI(result.data, session.doctorId);
          } else { localStorage.removeItem('doctorSession'); }
          if (btn) setLoading(btn, false);
      }).catch(() => { if (btn) setLoading(btn, false); });
