...
const processUserMessage = (rawMsg) => {
    ...
    if (cleanMsg.includes('نصيحة') || cleanMsg.includes('نصائح طبية') || cleanMsg === 'نصيحة') {
        return currentLang === 'ar'
    ...
    // ... باقي الكود الأصلي للبحث عن الأطباء ...
    ...
    detectedName = allDoctors.find(doc => { 
    ...
    if (detectedSpecialty && detectedMunicipality) {
        matchedDoctors = allDoctors.filter(doc => doc.specialty === detectedSpecialty && doc.municipality === detectedMunicipality);
